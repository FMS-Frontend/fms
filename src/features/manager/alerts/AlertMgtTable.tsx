...
const AlertMgtTable: FC<AlertTableProps> = ({ headingData, data }) => {
  ...
  const filteredData = data.filter((alert) => {
    const matchesSeverity =
      selectedSeverity === "" || alert.severity.toLowerCase() === selectedSeverity;
    const matchesStatus =
    ...
    const matchesDateRange =
      alertDate >= dateRange.startDate && alertDate <= dateRange.endDate;

    return matchesSeverity && matchesStatus && matchesSearch && matchesDateRange;
  });
    ...
    <div className="mt-8">
      ...
      <Table columns={`grid grid-cols-${headingData.length} gap-4`}>
        ...
        {filteredData.map((alert, index) => (
          <AlertTableRow key={alert.id} index={index} {...alert} />
        ))}
